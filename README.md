...
需要交付两项相互支撑的成果：**一份可复用的切分策略评估模型，以及一份可运行的 A3 kernel 实现。** 先提出策略和预测，再完成实现，用实验检验并修正判断。评估模型可以是 skill、文档、代码或工具。

这里是题目的总体说明。配套的[习题包说明](kda_a3_exercise/README.md)给出具体接口和运行方式，[可执行约束](kda_a3_exercise/contract.json)与[检查器](kda_a3_exercise/check.py)定义正确性规则。当前题包版本为 `0.3.0`；本题主要要求实现前向过程。反向与训练链路属于可选扩展。
